...
import { CustomerReportService, ClientOnboardingReport } from '../../../../shared/services/reports/customer-report.service';
...
export class ClientOnboardingComponent implements OnInit, OnDestroy {
  ...
  data = signal<ClientOnboardingReport | null>(null);
            ...
            $(win.document.body)
                .prepend(`
                  <div style="text-align: center; margin-bottom: 20px;">
                    ...
                    <h2 style="font-size: 18px; text-transform: uppercase;">Client Onboarding Report</h2>
                  </div>
                `);
  ...
  generateReport() {
    ...
    this.reportService.getClientOnboardingReport(this.startDate, this.endDate, 1, 100000).subscribe({
      next: (res) => {
  ...
  getOnboardingPercents() {
    const d = this.data();
    if (!d) return { verified: 0, pending: 0, unverified: 0 };
    const total = d.kycVerified + d.kycPending + d.kycUnverified;
    if (total === 0) return { verified: 0, pending: 0, unverified: 0 };
    return {
      verified: (d.kycVerified / total) * 100,
      pending: (d.kycPending / total) * 100,
      unverified: (d.kycUnverified / total) * 100
    };
  }
}
